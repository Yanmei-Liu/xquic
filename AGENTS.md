...
6. **Validation mapping**: The agent decides whether build/test execution is needed for the task. When needed, follow `docs_ai/validation_guide.md` for the commands and smallest correct test mapping. Use the `validate` skill if available.
...
| **Build/test validation** | **`docs_ai/validation_guide.md`** + `validate` skill (if installed) |
